...
class CollisionCircle:
    ...
    def collide_with_circle(self, other):
        ...
        if distance <= r1 + r2: # Collision
            ...
            impulse = -(1 + restitution_coefficient) * reduced_mass * (v1 - v2).dot(collision_normal) * collision_normal
    ...
    def collide_with_wall(self, other):
        restitution_coefficient = 1
        collision_normal = other.normal()
        distance = (self.position - other.position).dot(collision_normal)
        ...
        if distance < r1: # Collision
            overlap = r1 - distance
            self.move_position(overlap)

            reduced_mass = m1 # Since m2 would be infinite is reduces to m1
            
            impulse = -(1 + restitution_coefficient) * reduced_mass * v1.dot(collision_normal) * collision_normal
